		...
		for (Columns col : tc) {
			if (col.getColumns_type().indexOf("QTL value") == -1) {
				if (col.getHeader().toLowerCase().indexOf(word1.toLowerCase()) != -1
						...
						|| col.getHeader().toLowerCase().indexOf(word4.toLowerCase()) != -1) {
					return true;
				}
			}
		}
